...
const ImageCropper: React.FC<ImageCropperProps> = ({
    ...
    reader.onload = () => {
      ...
      imageElement.onload = (e: Event) => {
        ...
        if (naturalWidth < MIN_DIMENSION || naturalHeight < MIN_DIMENSION) {
          setError(
            t.pages.profile.addProfilePhoto.errors.minDimension(MIN_DIMENSION),
          );
          setImgSrc("");
        } else {
          setError("");
  ...
  return (
    <>
      ...
      {!imgSrc ? (
        <div
          className={`flex w-full flex-col items-center ${!error ? "mt-[72px]" : ""}`}
        >
        ...
        <div
          className={`flex w-full flex-col items-center ${!error ? "mt-[28px]" : ""}`}
        >
          <ReactCrop
            aspect={ASPECT_RATIO}
            circularCrop
