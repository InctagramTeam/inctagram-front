...
export const Dropdown = ({ children, modal, className, ...rest }: DropdownProps) => {
  ...
  return (
    <DropdownContext.Provider value={{ open, setOpen }}>
      <RadixDropdown.Root open={open} onOpenChange={setOpen} modal={modal} {...rest}>
        {children}
      </RadixDropdown.Root>
...
function DropdownButton({ children, className, ...rest }: DropdownButton) {
  return (
    <RadixDropdown.Trigger
      asChild
      className={clsx(
        `cursor-pointer select-none rounded text-regular-text-16 bg-Dark-500
        hover:bg-Dark-300/60 hover:transition-all duration-150 ease-in-out
        focus-visible:outline-none data-[state=open]:bg-Dark-100/75`,
        className
      )}
      {...rest}
    >
...
type SeparatorProps = ComponentPropsWithoutRef<typeof RadixDropdown.Separator>
