  ...
  (
    ...
    const classes = {
      input: clsx(
        ...
        focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-opacity-50 focus:text-Light-900
    ...
    return (
      <div className={classes.textField}>
            ...
            (isVisible ? (
              ...
              <button
                className={clsx(
                  `_RightBtn_ absolute flex items-center text-Light-100/80 unset cursor-pointer top-1/2
                right-[12px] transform -translate-y-1/2 transition-colors ease-in-out delay-150
                focus:outline focus:outline-Primary-500 active:not:disabled:text-Light-100`,
                  disabled && `text-Dark-100`
                )}
                disabled={disabled}
                onClick={onVisible}
        ...
        {error && <span className={`text-small-text-12 text-Danger-500`}>{error}</span>}
      </div>
    )
  }
)
