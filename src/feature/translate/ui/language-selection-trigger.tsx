...
export const LanguageSelectionTrigger = memo(
  ({ currentValue }: Props): ReturnComponent => {
    const { sm } = useResponsive();
    const { t } = useTranslation();

    const currentTextValue = getLanguages(t).find(
    ...
    hover:text-Light-900 hover:border-Light-900 justify-between`,
      ...
      triggerInner: cn(
        !sm && `flex items-center gap-[12px]`,
        sm && `basis-[20px] shrink-0 h-[20px]`,
      ),
    };
